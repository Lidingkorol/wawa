		...
		<div class="main">
			<div class="allRecord flex-box flex-direction_column">
				<div class="item_hd flex-box flex-align_center">
					<span class="flex-item">今日到账</span>
					<span class="flex-item">今日佣金</span>
				</div>
				<div class="item_bd flex-box flex-align_center">
					<span class="flex-item">￥{{money.today_true_money}}</span>
					<span class="flex-item">￥{{money.today_send_money}}</span>
				</div>
			</div>
	...
	export default {
		...
		data () {
			return {
				...
				user:{},
				money:{
					today_true_money:null,
					today_send_money:null
				}
			}
		},
		created() {
		...
		methods: {
			async getMyRecord(){
        		...
        		if(res.status == 200 && !!res.data){
	        		...
	        		this.money.today_true_money=res.data.today_true_money;
	        	}else {
	        		Toast(res.msg)
	        	}
			},
